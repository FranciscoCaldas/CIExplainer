...
def mc_l_shapley(
    ...
    for _ in range(sample_num):
        ...
        set_exclude_masks.append(set_exclude_mask)
        set_include_masks.append(set_include_mask)
    if len(set_exclude_masks) == 0: # TODO: check if this is the right way to handle this
        set_exclude_masks = [np.ones(num_nodes) for _ in range(sample_num)]
        set_include_masks = [np.ones(num_nodes) for _ in range(sample_num)]
    exclude_mask = np.stack(set_exclude_masks, axis=0)
    include_mask = np.stack(set_include_masks, axis=0)
    marginal_contributions = marginal_contribution(
        data, exclude_mask, include_mask, value_func, subgraph_build_func
...
def GnnNetsGC2valueFunc(gnnNets, target_class):
    ...
    def value_func(batch, **kwargs):
        with torch.no_grad():
            logits = gnnNets(batch.x, batch.edge_index, batch=batch.batch, **kwargs)
            probs = torch.softmax(logits, dim=-1)
            score = probs[:, target_class] if probs.size(1) > 1 else probs
        return score
...
def GnnNetsNC2valueFunc(gnnNets_NC, node_idx, target_class) -> Callable:
    ...
    def value_func(batch) -> torch.Tensor:
        with torch.no_grad():
            logits = gnnNets_NC(batch.x, batch.edge_index, batch=batch.batch)
            probs = F.softmax(logits, dim=-1)
            batched_probs = probs.view(batch.batch_size, -1, probs.shape[-1])
            score = batched_probs[:, node_idx, target_class] if batched_probs.size(2) > 1 else batched_probs[:, node_idx]
            return score
